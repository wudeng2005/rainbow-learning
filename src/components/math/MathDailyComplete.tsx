...
interface MathDailyCompleteProps {
...
export default function MathDailyComplete({
  questionsCorrect, totalQuestions, gemsEarned, isPractice, onPlayAgain,
}: MathDailyCompleteProps) {
  const navigate = useNavigate()
  const stars = totalQuestions > 0 ? Math.max(1, Math.round((questionsCorrect / totalQuestions) * 3)) : 3

  return (
    <div className="min-h-dvh flex flex-col items-center justify-center bg-gradient-to-b from-pink-100 via-rose-50 to-emerald-50 px-6 relative overflow-hidden">
      {/* 飘落庆祝 emoji */}
      {['🍬', '🍭', '⭐', '🧁', '🎈', '✨', '🌈', '🎉'].map((emoji, i) => (
        <motion.span
          key={i}
      ...
      >
        ...
        {!isPractice && gemsEarned > 0 && (
          <motion.div
            className="inline-flex items-center gap-2 bg-white/90 rounded-full px-5 py-2.5 mb-6 shadow-md border-2 border-pink-100"
            initial={{ y: 20, opacity: 0 }}
        ...
        <div className="flex flex-col gap-3 w-full max-w-[240px] mx-auto">
          <motion.button
            type="button"
            className="px-8 py-4 rounded-2xl bg-gradient-to-r from-pink-400 to-rose-400 text-white font-bold text-lg border-[3px] border-white/40 shadow-[0_5px_0_rgba(190,80,120,0.3)] active:shadow-none active:translate-y-1 touch-manipulation"
            whileTap={{ scale: 0.95 }}
            onClick={onPlayAgain}
          >
            🍭 再来一次
          ...
          <motion.button
            type="button"
            className="px-8 py-3.5 rounded-2xl bg-white text-text-secondary font-bold border-[3px] border-pink-100 shadow-[0_4px_0_rgba(190,80,120,0.1)] active:shadow-none active:translate-y-1 touch-manipulation"
            whileTap={{ scale: 0.95 }}
            onClick={() => navigate('/')}
          >
            回到首页
