...
## 更新摘要
**所做更改**   
- 新增了故事音频生成功能的详细说明，包括 generate-story-audio.py 脚本的使用
- 新增了字符信息生成功能的说明，包括 generate-character-info.py 脚本的作用
- 更新了运行时播放器部分，新增了 playCharAudio() 和 playStoryAudio() 函数
- 完善了中文学习组件的音频播放集成说明
- 扩展了项目结构图以包含新的故事音频和字符信息功能
...
- **增强运行时播放器**：统一封装 Web Audio API 音效与预录语音播放，提供英文发音兜底能力，并新增汉字读音和故事朗读播放函数。
...
**新增功能**
- **playCharAudio(audioPath)**：播放指定路径的汉字读音音频，支持空路径处理和错误恢复
- **playStoryAudio(storyId, sentenceIndex)**：根据故事ID和句子索引播放对应的故事音频
...
关键实现要点
- **音频集成**：通过 playCharAudio() 函数播放 /audio/chars/{char}.mp3 格式的音频文件。
...
关键实现要点
- **故事音频集成**：通过 playStoryAudio() 函数播放 /audio/stories/{storyId}_s{sentenceIndex}.mp3 格式的音频文件。
...
CharacterTeach["CharacterTeach.tsx"] --> sounds_ts
StoryPlayer["StoryPlayer.tsx"] --> sounds_ts
```
...
- **异步播放控制**
  - 新增的 playCharAudio() 和 playStoryAudio() 函数返回 Promise，支持精确的播放状态管理和用户体验优化。
...
## 结论
本项目通过"预生成 + 静态资源 + 运行时兜底"的音频方案，在保证稳定性的同时兼顾了用户体验。脚本层与数据源解耦，运行时播放逻辑统一封装，便于扩展与维护。**新增的故事音频和字符信息生成功能，以及增强的播放函数，显著提升了中文学习体验**。特别针对5岁儿童用户的英语音频参数优化，以及故事讲述的专用语音设置，使学习内容更加生动有趣。遵循"只补新增、先删旧再重录"的规则，可高效迭代音频内容。
